...
# Figure output ---------------------------------------------------------------
OUTPUT_PATH = "snia_3d_surface_merged.png"
FIG_WIDTH   = 9.0
FIG_HEIGHT  = 6.5
...
def make_figure(combined, band):
    ...
    print('this is the colour_map type', type(colour_map), colour_map)
    print('this is SN_colormap',cmap)
    ...
    legend_handles = []
    colours_manual = ['silver', 'forestgreen', 'chocolate', 'darkorchid', 'blue', 'chartreuse','gold', 'black', ]
    i = -1
    for sn, grp in combined.groupby('sn_name'):
        #col = colour_map[sn]
        i = i+1
        col = colours_manual[i]
        ...
        if not grp_model.empty:
            ...
            ax.plot(
                ...
                color=col, linewidth=1, alpha=0.5, zorder=8,
            )
    ...
    style_handles = [
        plt.Line2D([0], [0], marker=DATA_MARKER, color='w',
                   ...
                   markeredgewidth=0.4, markersize=10, label='Observed data'),
    ...
    for sn, grp in combined.groupby('sn_name'):
        ...
        ax.scatter(p_max, d_max, mag_peak,
                   s=60, color=col, depthshade=False,
                   alpha=1.0, 
                   marker='*', label = 'Peak magnitude')
...
# --- Legends attached to the figure, not the 3D axes -------------------
    # This sidesteps the broken bbox_to_anchor behaviour on Axes3D entirely.

    if n_sne <= 30:
        leg1 = fig.legend(
            handles=legend_handles,
            loc='center left',
            bbox_to_anchor=(0.88, 0.52),   # (x, y) in figure coords [0–1]
            fontsize=6,
            framealpha=0,
            markerscale=1.2,
            title='SN',
    ...
    fig.legend(
        ...
        loc='upper left',
        bbox_to_anchor=(0.88, 0.88),       # sits above the SN name legend
        fontsize=8,
        framealpha=0,
        borderpad=0.4,
    )

    # Shrink the 3D axes to leave a right-hand column for the legends --------
    ax.set_position([0.0, 0.0, 0.78, 1.0])   # [left, bottom, width, height]
    ax.yaxis.set_major_locator(plt.MaxNLocator(6))    
    ax.zaxis.set_major_locator(plt.MaxNLocator(6))    
    ax.xaxis.set_major_locator(plt.MaxNLocator(6))    

    # --- Labels & limits (unchanged) ----------------------------------------
    ax.set_xlabel(r'$t - T_0$ (days)', labelpad=10,  fontsize=16)
    ax.set_ylabel(r'$dm_{15}$',     labelpad=10,  fontsize=16)
    ax.set_zlabel(r'absolute magnitude', labelpad=14, fontsize=16)
