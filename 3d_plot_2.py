...
Input:  - one CSV (.csv) file per SN, each containing all bands
        - a separate lookup file (.xlsx) with SN parameters
Output: a single figure with one subplot for the chosen band

Surface fitting strategy
------------------------
Raw apparent magnitudes span many magnitudes across SNe at different distances,
which causes the spline to produce unphysical wiggles. The surface is therefore
fitted on peak-normalised relative magnitudes (mag - mag_peak per SN), which
captures the lightcurve shape family as in Burns+2011. The scatter points are
plotted in raw apparent magnitudes, each SN in a distinct colour.
...
def load_lookup(lookup_path, label_col, dm15_row, tmax_row, prefix, suffix):
    """Return dm15_map and tmax_map dicts keyed by filename stem."""
...
def load_all_data(data_dir, file_pattern, dm15_map, tmax_map, cfg):
    ...
    Load all CSV lightcurve files. Returns a DataFrame with columns:
        phase, mag, rel_mag, dm15, err, band, sn_name
    rel_mag = mag - mag_peak (per SN, used for surface fitting only).
    mag     = raw apparent magnitude (used for scatter plotting).
    """
    ...
    for fp in filepaths:
        ...
        try:
            ...
            # Relative magnitude for surface fitting (removes distance offset)
            df["rel_mag"] = df["mag"] - df["mag"].min()

            frames.append(df)
            print(f"  Loaded : {os.path.basename(fp)}"
                  f"  ({len(df)} pts, dm15={dm15_val:.3f}, t_max={tmax_val:.2f})")
...
def make_figure(combined, band, cfg):
    """
    3D figure: both surface and scatter plotted on rel_mag = mag - mag_peak,
    so all SNe share a common peak level (same convention as Burns+2011).
    z-axis is inverted so peak (rel_mag = 0) sits at the top.
    """
    ...
    # -- Smooth surface (fitted on rel_mag) -----------------------------------
    phase_all   = combined["phase"].values.astype(float)
    dm15_all    = combined["dm15"].values.astype(float)
    rel_mag_all = combined["rel_mag"].values.astype(float)

    if len(phase_all) >= 16:
        try:
            P, D, Z = fit_surface(
                phase_all, dm15_all, rel_mag_all,
    ...
    ax.set_zlabel(r"$m - m_{\mathrm{peak}}$ (mag)",  labelpad=10, fontsize=10)
    ax.set_title(f"{band} band", fontsize=11, pad=4)

    ax.set_xlim(cfg["phase_min"], cfg["phase_max"])
    ax.set_ylim(cfg["dm15_min"],  cfg["dm15_max"])

    # Invert z: peak (rel_mag = 0) at top, decline falls downward
    all_rel    = combined["rel_mag"].values
    z_top      = -0.2
    z_bottom   = np.nanmax(all_rel) + 0.3
