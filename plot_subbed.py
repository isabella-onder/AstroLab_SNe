...
SNes = ['2025acsn', '2025acsn_subbed'] #here, put the name of the supernova to analyse
zpt_plot_tick = False
star_nb = False
plain_zpt = False
psf = False
...
labels = ['I', 'R', 'V', 'B']
colours_ztf = ['darkseagreen', 'rosybrown', 'grey']
colours_ztf = ['grey', 'grey', 'grey', 'grey']
labels_ztf = ['g', 'r', 'i']
...
index = 0
fig, axs = plt.subplots(2,2, sharex = True, gridspec_kw={'hspace': 0.05,'wspace': 0.20})
axs_list = [axs[0,0], axs[0,1], axs[1,0],axs[1,1]]
filter = ['I', 'R', 'V', 'B']
for ax, filter in zip(axs_list, filter):
    ax.minorticks_on()
    ...
    ax.text(0.9, 0.85, filter, transform=ax.transAxes,
            fontsize=14, va="bottom", ha="left", color="black", fontweight='bold')

for overarching,colours,marker_shape, s_over in zip(overarchings,over_colours, markersover, over_size):
    index = 0
    for band,axs in zip(overarching,axs_list):
        ...
        axs.errorbar(time, mags,(mag_error_up, mag_error_down), fmt = marker_shape, color = colours[index],label = labels[index], markersize =s_over)
...
fig.supxlabel('Time (Modified Julian Days)', fontsize=13)
fig.supylabel('Apparent magnitude', fontsize=13)
    
from matplotlib.lines import Line2D

# After your plotting loop, before plt.show()
# After your plotting loop

ax = axs_list[0]  # just used to generate handles

colours  = ["#D7A929", 'firebrick', 'mediumseagreen', 'steelblue']


legend_elements = [
    ax.errorbar([], [], yerr=0.1, fmt='.', color="#D7A929",      markersize=8, capsize=3, label='Subtracted'),
    ax.errorbar([], [], yerr=0.1, fmt='.', color='firebrick',      markersize=8, capsize=3, label='R'),
    ax.errorbar([], [], yerr=0.1, fmt='.', color='mediumseagreen', markersize=8, capsize=3, label='V'),
    ax.errorbar([], [], yerr=0.1, fmt='.', color='steelblue', markersize=8, capsize=3, label='B'),
    ax.errorbar([], [], yerr=0.1, fmt='^', color='grey',           markersize=8, capsize=3, label='Pre-subtraction'),
    ax.errorbar([], [], yerr=0.1, fmt='+', color='grey',           markersize=8, capsize=3, label='ZTF'),
]
...
plt.savefig("Figures/before_v_after_acsn_subtraction.png", bbox_inches='tight', dpi=150)
plt.show()
